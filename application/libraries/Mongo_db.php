	...
	*/
	public function count($collection = ""){
		try {
			...
			$cursor = $this->db->executeCommand($this->database, $command, $read_preference);
			...
			return isset($result->count) ? (int) $result->count : 0;
				...
				if (isset($this->debug) && $this->debug == TRUE) {
						show_error("MongoDB count query failed: {$e->getMessage()}", 500);
				} else {
						show_error("MongoDB count query failed.", 500);
				}
		}
	}
...
/*	public function count($collection = "")
{
    try {
        ...
        $options = [
            'readConcern'    => new MongoDB\Driver\ReadConcern($this->read_concern),
            'readPreference' => new MongoDB\Driver\ReadPreference($this->read_preference),
        ];

        // Directly count documents (no aggregation)
        $count = $this->db->{$collection}->countDocuments($filter, $options);
	...
	*/
	public function distinct($collection = "", $field="")
	{
		...
		try
		{
			...
			$cursor = $this->db->executeCommand($this->database, $command);
			$scents = current($cursor->toArray())->values;

			// Clear wheres after query so next queries are fresh
			$this->wheres = [];
			$this->limit = 999999;
			$this->offset = 0;
			$this->sorts = [];
	...
	*/
	public function update($collection = "", $options = array())
	{
		...
		$bulk = new MongoDB\Driver\BulkWrite();
		$options = array_merge($options, array('multi' => true,'upsert'=>true));
	...
	*/
	public function aggregate($collection, $operation)
    {
        ...
        $command = array('aggregate'=>$collection, 'pipeline'=>$operation,'cursor'=>array('batchSize'=>0));
        $com= $this->command($command);
        return (object)array('result'=>json_decode(json_encode($com),TRUE));
	}
	...
	*/
	...
	public function findAndModify($collection){
		...
		$ret = $this->db->{$collection}->findAndModify($this->wheres,$this->updates);
		$this->_clear();
		return $ret;
	}

	public function incUpdate($collection,$coll){
        $read_concern    = new MongoDB\Driver\ReadConcern($this->read_concern);
        $read_preference = new MongoDB\Driver\ReadPreference($this->read_preference);
        try
        {
            $command=new MongoDB\Driver\Command([
                ...
                "query"=>$this->wheres,
                "update"=>array("\$inc"=>array($coll=>1)),
                "new"=> 1,
                "upsert"=> 1,
            ]);
            $cursor = $this->db->executeCommand($this->database, $command,$read_preference);
			$this->_clear();
            $scents = current($cursor->toArray()); // get the distinct values as an array
            return json_decode(json_encode($scents),TRUE)['value'];
        }
        catch (MongoCursorException $e)
        {
            if(isset($this->debug) == TRUE && $this->debug == TRUE)
